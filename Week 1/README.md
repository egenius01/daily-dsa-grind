...
1. **What did I learn today?**
   - Instead of checking rows, cols, and boxes in 3 separate passes, check them all at once in a single pass.
   - Mapping a coordinate (r, c) to a 3x3 sub-box using Integer Division: `(r // 3, c // 3)`.
   - Key "Senior" Pattern: Don't store data to check later. Validate immediately as you iterate using Hash Sets (`seen_rows`, `seen_cols`, `seen_boxes`).
2. **Where did I struggle?**
   - I initially tried to over-engineer the Sudoku storage (building lists before checking). Learned to trust the "Check-As-You-Go" pattern.
3. **What was the pattern?**
   - Hash Sets / Matrix Traversal
4. **Big (O) Notation**
   - Time Complexity: $O(1)$ (The board is a fixed 9x9 size).
   - Space Complexity: $O(1)$

## Problem-128 Longest Consecutive Sequence

1. **What did I learn today?**
   - Sorting takes $O(n \log n)$, which is forbidden for this problem. Use a Set for $O(1)$ lookups.
   - The "Start" Check: `if (n - 1) not in set`. This simple check ensures we only process each sequence once, preventing $O(n^2)$ behavior.
   - Mental Model: "Sets are for Checking Presence; Dictionaries are for Counting/Grouping."
2. **Where did I struggle?**
   - Implemented the $O(n)$ sequence logic correctly despite fatigue and power outages.
3. **What was the pattern?**
   - Hash Sets
4. **Big (O) Notation**
   - Time Complexity: $O(n)$
